...
const FormAddCourseUserData = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm();
  const { addUserPersonalData } = useAuth();
  ...
  const submitData = handleSubmit(async (data, e) => {
    ...
    if (data.fecha_nacimiento > fecha_actual_formateada) {
      ...
      if (res.status === 201) {
        AlertInfo({
          title: "Pre inscribete nuevamente presionando el botón del curso seleccionado anteriormente",
          timer: 150000,
        });
        navigate("/cursos");
      }
...
export default FormAddCourseUserData;
